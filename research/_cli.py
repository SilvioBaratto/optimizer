"""Argparse surface for ``research/stock_selection_pipeline.py`` (Cycle 5 §13).

Exposes the 9-flag CLI as a separate module so the parser is unit-testable
without importing the full pipeline.  The pipeline entry point at
``research/stock_selection_pipeline.py`` calls :func:`build_parser` then
forwards the resulting :class:`argparse.Namespace` to :func:`main`.
"""
...
from research.stock_selection_pipeline import N_SELECTED, REBALANCE_FREQ
