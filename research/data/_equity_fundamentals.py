"""Equity fundamentals, financial statements, analyst and insider assembly functions.
...
def _dedup_fundamentals_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[~df.index.duplicated(keep="first")]  # pyright: ignore[reportAssignmentType]
...
def _compute_asset_growth_from_statements(
    ...
    for instrument_id, _, value in rows:
