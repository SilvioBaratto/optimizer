...
down_revision: Union[str, Sequence[str], None] = 'ecf0a9a2bfdc'
...
def upgrade() -> None:
    ...
    # ----------------------------------------------------------------
    op.drop_constraint('instruments_exchange_id_fkey', 'instruments', type_='foreignkey')
    op.create_foreign_key(
        'instruments_exchange_id_fkey', 'instruments', 'exchanges',
    ...
    # ----------------------------------------------------------------
    op.create_index('ix_analyst_price_targets_instrument_id', 'analyst_price_targets', ['instrument_id'])

    # ----------------------------------------------------------------
    # 5. Add missing index on insider_transactions.start_date
    # ----------------------------------------------------------------
    op.create_index('ix_insider_transactions_start_date', 'insider_transactions', ['start_date'])

    # ----------------------------------------------------------------
    # 6. Add missing index on financial_statements.period_date
    # ----------------------------------------------------------------
    op.create_index('ix_financial_statements_period_date', 'financial_statements', ['period_date'])
    ...
    op.create_index('ix_trading_economics_indicators_country', 'trading_economics_indicators', ['country'])
...
def downgrade() -> None:
    ...
    op.add_column('bond_yields', sa.Column(
        'scraped_at', postgresql.TIMESTAMP(timezone=True),
        server_default=sa.text('now()'), nullable=False,
    ))
    op.add_column('trading_economics_indicators', sa.Column(
        'scraped_at', postgresql.TIMESTAMP(timezone=True),
        server_default=sa.text('now()'), nullable=False,
    ))
    op.add_column('economic_indicators', sa.Column(
        'scraped_at', postgresql.TIMESTAMP(timezone=True),
        server_default=sa.text('now()'), nullable=False,
    ))
    ...
    op.drop_index('ix_trading_economics_indicators_country', table_name='trading_economics_indicators')
    ...
    op.drop_index('ix_financial_statements_period_date', table_name='financial_statements')

    # 5. Drop insider_transactions.start_date index
    op.drop_index('ix_insider_transactions_start_date', table_name='insider_transactions')

    # 4. Drop analyst_price_targets.instrument_id index
    op.drop_index('ix_analyst_price_targets_instrument_id', table_name='analyst_price_targets')
