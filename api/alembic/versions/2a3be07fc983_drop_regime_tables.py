...
def upgrade() -> None:
    ...
    op.drop_index('ix_regime_state_probs_state_id', table_name='regime_state_probabilities')
    op.drop_constraint('uq_regime_state_prob_regime', 'regime_state_probabilities', type_='unique')
...
def downgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('updated_at', sa.DateTime(), nullable=False),
        sa.ForeignKeyConstraint(['regime_state_id'], ['regime_states.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
