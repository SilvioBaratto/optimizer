...
def validate_factors(
    ...
    if validation_type == "out_of_sample":
        report_data = _run_oos_validation(
            scores, returns_history, factor_type, end_date
        )
    else:
        report_data = _run_in_sample_validation(
            scores, returns_history, factor_type, end_date
        )
...
def compute_factor_scores(
    ...
    """
    repo = FactorRepository(session)
    raw_scores = repo.get_scores_by_tickers_at_date(
        tickers=tickers, score_date=score_date
    )
    ...
    if composite_method in _IC_METHODS:
        ...
        extra.update(
            _build_training_data(
                repo, tickers, training_start_date, training_end_date, session
            )
        )
    return extra
...
def _build_training_data(
    ...
    standardized_training = [
        s for s in training_raw if s.standardized_score is not None
    ]
    if not standardized_training:
        return {}

    training_df, _ = _build_standardized_scores_df(standardized_training)
    price_rows = _fetch_price_rows(
        session, tickers, training_start_date, training_end_date
    )
