...
def _run_selection(
    ...
    return {
        ...
        "count": len(selected),
        "turnover": turnover if prev_members is not None and len(prev_members) > 0 else None,
...
def _build_selection_config(
    ...
    sel_method = SelectionMethod.QUANTILE if method == "quantile" else SelectionMethod.FIXED_COUNT
...
def _compute_buffer_zone(
    ...
    return list(selected.difference(current_members)), list(current_members.difference(selected))
...
def compute_quintile_spread_for_tickers(
    ...
    repo = FactorRepository(session)
    raw_scores = repo.get_scores_by_tickers_and_date_range(
        tickers=tickers, start_date=start_date, end_date=end_date, factor_type=factor_name
    )
    if not raw_scores:
        raise FactorDataError(
    ...
    result = compute_quintile_spread(scores=scores_df, returns=returns_df, n_quantiles=n_quantiles)
    return {
        "quintile_cumulative_returns": _cumulative_returns_dict(result.quintile_returns),
...
def _cumulative_returns_dict(quintile_returns: pd.DataFrame) -> dict[str, list[float]]:
    ...
    return {
        str(col): [round(float(v), 8) for v in (1 + quintile_returns[col].fillna(0)).cumprod() - 1]
...
def compute_regime_tilt(
    ...
    if macro_df.empty:
        raise FactorDataError("No macro data found in database for regime classification")
...
def _fetch_macro_dataframe(session: Session) -> pd.DataFrame:
    ...
    df = pd.DataFrame(rows)
    pivot = df.pivot_table(index="date", columns="indicator_key", values="value", aggfunc="last")
...
def _apply_tilts(
    ...
    tilted = apply_regime_tilts(group_weights=typed_weights, regime=regime, config=tilt_config)
    raw_tilts = get_regime_tilts(regime, tilt_config)
    return {
        "regime": regime.value,
        "tilted_weights": {g.value: float(w) for g, w in tilted.items()},
        "tilt_multipliers": {g.value: float(raw_tilts.get(g, 1.0)) for g in typed_weights},
    }
