...
from app.schemas.universe.universe_screen import ScreenPreset, UniverseScreenRequest
...
def _assemble_fundamentals(session: Session) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        rows, columns=["yfinance_ticker", "market_cap", "current_price", "exchange"]
    )
...
def _apply_overrides(
    ...
    return InvestabilityScreenConfig(
        ...
        trading_frequency=_hysteresis(
            request.trading_frequency, base.trading_frequency
        ),
        ...
        min_quarterly_reports=request.min_quarterly_reports
        or base.min_quarterly_reports,
        exchange_region=_region(request.exchange_region),
        mcap_percentile_entry=request.mcap_percentile_entry
        or base.mcap_percentile_entry,
...
def _run_market_cap_screens(
    ...
    results["market_cap"] = _passing_set(
        apply_screen(mcap, config.market_cap, current_members)
    )
...
def _compute_mcap_percentile_pass(
    ...
    for _, group_mcaps in mcaps.groupby(exchanges):
        if len(group_mcaps) >= min_size:
            entry_thresh = float(
                np.percentile(group_mcaps.values, config.mcap_percentile_entry * 100)
            )
            exit_thresh = float(
                np.percentile(group_mcaps.values, config.mcap_percentile_exit * 100)
            )
        else:
...
def _run_liquidity_screens(
    ...
    results["addv_12m"] = _passing_set(
        apply_screen(addv_12.dropna(), config.addv_12m, current_members)
    )
    addv_3 = compute_addv(close_df, volume_df, window=63)
    results["addv_3m"] = _passing_set(
        apply_screen(addv_3.dropna(), config.addv_3m, current_members)
    )
    freq = compute_trading_frequency(volume_df, window=252)
    results["trading_frequency"] = _passing_set(
        apply_screen(freq.dropna(), config.trading_frequency, current_members)
    )
