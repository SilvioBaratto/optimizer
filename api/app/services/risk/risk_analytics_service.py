...
class RiskAnalyticsService:
    ...
    def compute_var(
        ...
        """
        logger.debug(
            "compute_var portfolio_id=%s method=%s lookback=%d",
            portfolio_id,
            method,
            lookback,
        )
    ...
    def compute_correlation(
        ...
        """
        logger.debug(
            "compute_correlation portfolio_id=%s lookback=%d", portfolio_id, lookback
        )
    ...
    def compute_liquidity(
        ...
        assets = [
            _build_liquidity_asset(
                ...
                weight,
                names,
                self._fetch_addv(ticker, lookback_days),
                participation_rate,
            )
    ...
    def _fetch_prices(self, weights: dict[str, float], lookback: int) -> pd.DataFrame:
...
def _build_concentration_assets(
    ...
    entries = [
        {"ticker": t, "name": names.get(t, t), "weight": w} for t, w in weights.items()
    ]
...
def _compute_concentration_summary(
    ...
    return {
        "hhi": round(hhi, 8),
        "effective_n": round(effective_n, 8),
        "top_n_ratio": round(top_n_ratio, 8),
    }
...
def _compute_liquidity_summary(
    ...
    valid = [
        (a["weight"], a["days_to_liquidate"])
        for a in assets
        if a["days_to_liquidate"] is not None
    ]
    if not valid:
