...
class PdfBuilder:
    ...
    def build(
        ...
        for section_id, data in section_data.items():
            title = self._SECTION_TITLES.get(
            ...
            if not data.get("available", False):
                story.append(
                    Paragraph(
                        data.get("message", "No data available"), styles["Normal"]
                    )
                )
            else:
    ...
    def _render_weights(self, story: list, styles: Any, data: dict[str, Any]) -> None:
        ...
        for item in items:
            table_data.append(
                [item.get("ticker", ""), f"{item.get('weight', 0) * 100:.2f}"]
            )
    ...
    def _render_generic(self, story: list, styles: Any, data: dict[str, Any]) -> None:
