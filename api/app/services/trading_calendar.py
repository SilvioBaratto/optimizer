...
def parse_period_years(period: str) -> Optional[int]:
...
def get_expected_trading_sessions(
    ...
    try:
        ...
        cal_start = cal.first_session.date() if hasattr(cal.first_session, 'date') else cal.first_session
        cal_end = cal.last_session.date() if hasattr(cal.last_session, 'date') else cal.last_session
        ...
        logger.warning(
            "Failed to compute sessions for %s (MIC=%s)", exchange_name, mic,
            exc_info=True,
        )
        return None
