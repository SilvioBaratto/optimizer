    ...
    **Returns 500** when the entropy pooling solver fails to converge.
    ...
    try:
        result = run_entropy_pooling(
            ...
            variance_views=tuple(request.variance_views)
            if request.variance_views
            else None,
            correlation_views=tuple(request.correlation_views)
            if request.correlation_views
            else None,
            skew_views=tuple(request.skew_views) if request.skew_views else None,
            kurtosis_views=tuple(request.kurtosis_views)
            if request.kurtosis_views
            else None,
