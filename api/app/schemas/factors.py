...
class FactorScoreRequest(BaseModel):
    ...
    score_date: datetime.date = Field(..., description="Date for which to compute composite scores")
...
class FactorValidateResponse(BaseModel):
    ...
    ic_mean: float | None = Field(default=None, description="Mean information coefficient")
    ic_std: float | None = Field(default=None, description="Standard deviation of IC")
    icir: float | None = Field(default=None, description="IC information ratio")
    t_stat: float | None = Field(default=None, description="Newey-West t-statistic")
    p_value: float | None = Field(default=None, description="p-value for the t-statistic")
    vif: float | None = Field(default=None, description="Variance inflation factor")
...
class BufferZone(BaseModel):
    ...
    entered: list[str] = Field(default_factory=list, description="Tickers that newly entered")
    exited: list[str] = Field(default_factory=list, description="Tickers that were dropped")
...
class FactorSelectResponse(BaseModel):
    ...
    buffer_zone: BufferZone = Field(..., description="Tickers entering/exiting via buffer")
...
class FactorQuintileSpreadRequest(BaseModel):
    ...
    factor_name: str = Field(..., description="Factor type to analyse (e.g. momentum_12_1)")
...
class FactorRegimeTiltRequest(BaseModel):
    ...
    group_weights: dict[str, float] = Field(
        ..., min_length=1, description="Base group weights keyed by FactorGroupType value"
    )
