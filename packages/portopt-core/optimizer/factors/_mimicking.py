...
def build_all_factor_mimicking_portfolios(
    ...
    for name, scores in factor_scores.items():
        fmp = build_factor_mimicking_portfolios(
            scores,
            returns,
            quantile=quantile,
            weighting=weighting,
        ...
        columns[name] = fmp["factor_return"].rename(name)

    # pd.DataFrame over a dict of Series outer-aligns on the union of indices
    # and preserves insertion order of the columns.
