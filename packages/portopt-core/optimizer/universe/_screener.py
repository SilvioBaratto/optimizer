...
def compute_exchange_mcap_percentile_thresholds(
    ...
    for _, group_mcaps in mcaps.groupby(exchanges):
        # Count only finite market caps toward the exchange-size gate and use a
        # NaN-aware percentile so a single missing/invalid mcap does not poison
        # the threshold for every stock on the exchange (which would silently
        # exclude the whole exchange from the universe).
        valid = group_mcaps.to_numpy(dtype=float)
        n_valid = int(np.isfinite(valid).sum())
        if n_valid >= min_exchange_size:
            threshold = float(np.nanpercentile(valid, percentile * 100))
