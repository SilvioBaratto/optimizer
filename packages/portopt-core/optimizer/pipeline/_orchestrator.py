...
from optimizer.preprocessing._returns import ReturnsConfig, to_returns
...
def tune_and_optimize(
    ...
    *,
    ...
    # Tidy per-candidate search results (issue: surface tuning diagnostics).
    search_results: pd.DataFrame | None
    try:
        search_results = search_results_dataframe(gs)
    except (KeyError, AttributeError, ValueError) as exc:
        logger.warning("Could not build search results dataframe: %s", exc)
        search_results = None
...
def run_full_pipeline(
    ...
    *,
    ...
    returns_config : ReturnsConfig or None
        Controls how prices are converted to linear returns.  When
        ``None`` (default), the plain ``skfolio.prices_to_returns`` path
        is used (fully backward-compatible).  When provided,
        :func:`optimizer.preprocessing.to_returns` is used instead,
        which additionally handles NaN thresholds, inception dropping,
        join method, and — when ``y_prices`` is given — joint alignment
        of asset and benchmark returns on a shared index.
    expiration_dates : dict[str, datetime.datetime] or None
        Ticker → expiration date, forwarded to the pre-selection
        pipeline's ``SelectNonExpiring`` step.  Only takes effect when
        ``pre_selection_config.use_non_expiring`` is set with a positive
    ...
    # 1. Prices → returns
    #     Backward-compatible default: plain skfolio prices_to_returns.
    #     When a ReturnsConfig is supplied, delegate to to_returns which
    #     additionally handles NaN thresholds / inception dropping / join
    #     method and — with y_prices — jointly aligns X and y on one index.
    y: pd.DataFrame | None
    if returns_config is not None:
        if y_prices is not None:
            _xy = to_returns(prices, y_prices, config=returns_config)
            X, y = cast("tuple[pd.DataFrame, pd.DataFrame]", _xy)
        else:
            X = cast(pd.DataFrame, to_returns(prices, config=returns_config))
            y = None
    else:
    ...
    # 4b. Summarise CV/backtest fold failures (skfolio FailedPortfolio
    #      sentinels emitted when an optimizer with raise_on_failure=False /
    #      fallback= cannot solve a rebalance).  Purely diagnostic; never
    #      raises so it cannot break an otherwise successful run.
    if bt is not None:
        try:
            cv_failures = summarize_cv_failures(bt)
        except Exception:
            # Diagnostics must never break an otherwise successful run.
            logger.warning("summarize_cv_failures failed; skipping.", exc_info=True)
        else:
            result.cv_failures = cv_failures
            if cv_failures.has_failures:
                logger.warning(
                    "Backtest had %d/%d failed fold(s) (%.1f%%); see "
                    "result.cv_failures for per-fold error messages.",
                    cv_failures.n_failed,
                    cv_failures.total,
                    cv_failures.failure_rate * 100.0,
                )
...
def run_full_pipeline_with_selection(
    ...
    *,
    ...
    returns_config : ReturnsConfig or None
        Price→returns conversion policy forwarded verbatim to
        :func:`run_full_pipeline`.  ``None`` (default) keeps the plain
        ``prices_to_returns`` path.
    expiration_dates : dict[str, datetime.datetime] or None
        Ticker → expiration date forwarded verbatim to
        :func:`run_full_pipeline` (drives the pre-selection
        ``SelectNonExpiring`` step).  ``None`` (default) retains every
        asset.
    n_jobs : int or None
        Number of parallel jobs.
