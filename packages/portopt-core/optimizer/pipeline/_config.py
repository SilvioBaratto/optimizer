...
if TYPE_CHECKING:
    ...
    from optimizer.validation._factory import CrossValFailureReport
...
class PortfolioResult:
    ...
    weight_history : pd.DataFrame or None
        ...
        ``None`` when no backtest was run.
    cv_failures : CrossValFailureReport or None
        Summary of failed folds in the walk-forward backtest, produced by
        :func:`optimizer.validation.summarize_cv_failures`.  Counts any
        ``skfolio.portfolio.FailedPortfolio`` sentinels emitted when an
        optimizer configured with ``raise_on_failure=False`` / ``fallback=``
        cannot solve a rebalance.  ``None`` when no backtest was run.
    search_results : pd.DataFrame or None
        Tidy hyperparameter-search results from
        :func:`optimizer.tuning.search_results_dataframe` (one row per
        candidate, mean/std test score, rank).  Populated only by
        :func:`optimize.tune_and_optimize`; ``None`` otherwise.
    """
    ...
    cv_failures: CrossValFailureReport | None = None
