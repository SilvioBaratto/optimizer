...
// ─── Suite: analytics endpoints keyed by context + market independence (#981) ──
// Wire-level lock: with the real DashboardService + HttpTestingController, the
// five analytics endpoints must each be requested with the active context key,
// and the market snapshot must load even when no portfolio is selected.

describe('DashboardComponent — analytics endpoints keyed by context (issue #981)', () => {
  let http: HttpTestingController;
  let ctx: PortfolioContextService;
  const API = environment.apiUrl;
  const PORTFOLIO = 'ctx-key';
  ...
  beforeEach(async () => {
    ...
    ctx.reset(); // clear any id bled in from a prior spec in the Karma session
    http = TestBed.inject(HttpTestingController);
  });

  afterEach(() => http.verify());

  it('when a portfolio is selected, each of the 5 analytics endpoints is requested with the context key', () => {
    ctx.currentPortfolioId.set(PORTFOLIO);
    TestBed.createComponent(DashboardComponent).detectChanges();

    const endpoints = ['performance-metrics', 'equity-curve', 'allocation', 'asset-class-returns', 'rolling-metrics'];
    for (const ep of endpoints) {
      const reqs = http.match((r) => r.url === `${API}portfolio-analytics/${PORTFOLIO}/${ep}`);
      expect(reqs.length).withContext(ep).toBeGreaterThanOrEqual(1);
    }
    drainAll();
  });

  it('when no portfolio is selected, the market snapshot request still fires', () => {
    TestBed.createComponent(DashboardComponent).detectChanges();

    const snap = http.match((r) => r.url === `${API}market/snapshot`);
