...
import { computeDrawdownSeries } from './drawdown/drawdown';
...
export class DashboardComponent implements OnDestroy {
  ...
  private loadPortfolioData(): void {
    ...
    this.dashboardSvc
      .loadPortfolioData(name, this.period(), this.benchmark())
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (snapshot) => {
          this.kpis.set(snapshot.kpis);
          this.nav.set(snapshot.nav);
          this.dailyChange.set(snapshot.navChangePct);
          this.currency.set(snapshot.currency);
          this.equityCurve.set(snapshot.equityCurvePoints);
        ...
        error: (err: Error) => {
          ...
          this.errorMessage.set(err?.message ?? 'Failed to load portfolio data');
          this.isLoadingPortfolio.set(false);
        },
      });
  }
  ...
  formatKpiValue(kpi: DashboardKPI): string {
    return this.dashboardSvc.formatKpiValue(kpi, this.currency());
  }

  kpiTrend(kpi: DashboardKPI): 'up' | 'down' | 'flat' {
    return this.dashboardSvc.kpiTrend(kpi);
  }

  kpiDelta(kpi: DashboardKPI): number {
    return this.dashboardSvc.kpiDelta(kpi, this.nav());
  }
  ...
  kpiDeltaFormat(kpi: DashboardKPI): 'percent' | 'absolute' {
    return this.dashboardSvc.kpiDeltaFormat(kpi);
  }
