...
class StockSignal(Base, TimestampMixin):
    ...
    signal_date: Mapped[date] = mapped_column(
        ...
        nullable=False,
        index=True,
        comment="Date when the signal was generated"
    )
    ...
    close_price: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Closing price on signal date"
    )

    open_price: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Opening price on signal date"
    )

    daily_return: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Daily return percentage (close-to-close)"
    )

    volume: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Trading volume on signal date"
    )
    ...
    rsi: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Relative Strength Index (RSI) indicator"
    )
    ...
    data_quality_score: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Data quality score (0-1) for signal reliability"
    )
    ...
    momentum_score: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Momentum score: -1 (downtrend) to +1 (uptrend)"
    )

    quality_score: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Quality score: 0 (poor) to 1 (excellent)"
    )
    ...
    technical_score: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Technical score: -1 (bearish) to +1 (bullish)"
    )
    ...
    upside_potential_pct: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Estimated upside % to fair value or target"
    )

    downside_risk_pct: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Estimated downside % risk to support levels"
    )
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            'instrument_id',
            'signal_date',
            name='uq_stock_signals_instrument_date'
        ),
        ...
        Index('idx_stock_signals_date_sector_type', 'signal_date', 'sector', 'signal_type'),
