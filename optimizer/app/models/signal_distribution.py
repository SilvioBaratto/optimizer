...
class SignalDistribution(Base, TimestampMixin):
    ...
    sample_size: Mapped[int] = mapped_column(
        Integer,
        nullable=False,
        comment="Number of stocks in this distribution sample"
    )
    ...
    p20: Mapped[float] = mapped_column(
        Float,
        nullable=False,
        comment="20th percentile threshold (bottom 20%)"
    )
    ...
    p80: Mapped[float] = mapped_column(
        Float,
        nullable=False,
        comment="80th percentile threshold (top 20%)"
    )
    ...
    p20_deviation: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Deviation from theoretical p20 (-0.84)"
    )

    p40_deviation: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Deviation from theoretical p40 (-0.25)"
    )

    p60_deviation: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Deviation from theoretical p60 (+0.25)"
    )

    p80_deviation: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Deviation from theoretical p80 (+0.84)"
    )
    ...
    min_zscore: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Minimum z-score in sample"
    )

    max_zscore: Mapped[Optional[float]] = mapped_column(
        Float,
        nullable=True,
        comment="Maximum z-score in sample"
    )
    ...
    def from_zscore_history(
        ...
        bucket_pcts = [large_gain_pct, small_gain_pct, neutral_pct, small_decline_pct, large_decline_pct]
        ...
        if not np.isnan(max_bucket_deviation) and max_bucket_deviation > 10.0:
            is_valid = False
            validation_notes.append(f"Bucket imbalance: {max_bucket_deviation:.1f}% (expected < 5%)")
