...
from sqlalchemy import (
    String, Date, Integer, Numeric, ForeignKey, Index, JSON, Enum as SQLEnum
)
...
class Portfolio(Base, TimestampMixin):
    ...
    total_positions: Mapped[int] = mapped_column(
        Integer,
        nullable=False,
        comment="Number of positions in portfolio"
    )
...
class PortfolioPosition(Base, TimestampMixin):
    ...
    ticker: Mapped[str] = mapped_column(
        String(50),
        nullable=False,
        comment="Trading212 ticker (e.g., 'NFLX_US_EQ')"
    )

    yfinance_ticker: Mapped[Optional[str]] = mapped_column(
        String(50),
        nullable=True,
        comment="Yahoo Finance ticker (e.g., 'NFLX')"
    )
    ...
    price: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(20, 6),
        nullable=True,
        comment="Stock price at portfolio creation"
    )
    ...
    conviction_tier: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        comment="Conviction tier: 1 (highest), 2, 3 (lowest)"
    )
    ...
    equilibrium_return: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 6),
        nullable=True,
        comment="Equilibrium expected return (prior)"
    )

    posterior_return: Mapped[Optional[Decimal]] = mapped_column(
        Numeric(10, 6),
        nullable=True,
        comment="Posterior expected return (after views)"
    )
    ...
    selection_reason: Mapped[Optional[str]] = mapped_column(
        String(500),
        nullable=True,
        comment="Why this stock was selected/weighted"
    )
