...
class NewsArticle(BaseModel):
    ...
    publisher: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="Publisher name"
    )

    published_date: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, index=True, comment="Publication date"
    )

    link: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="URL to article"
    )

    # Relationships
    assessment: Mapped["CountryRegimeAssessment"] = relationship(
        "CountryRegimeAssessment", back_populates="news_articles"
    )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<NewsArticle(title='{title_preview}...', publisher='{self.publisher}')>"
        )
