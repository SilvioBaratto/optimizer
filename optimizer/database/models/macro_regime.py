...
if TYPE_CHECKING:
    from optimizer.database.models.news import NewsArticle
# ============================================================================
...
class MacroAnalysisRun(BaseModel):
    ...
    num_countries: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="Number of countries analyzed"
    )
    ...
    notes: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Optional notes"
    )
...
class MarketIndicators(BaseModel):
    ...
    ism_pmi: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="ISM Manufacturing PMI"
    )

    yield_curve_2s10s: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="2s10s yield spread (bps)"
    )

    hy_spread: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="High-yield credit spread (bps)"
    )

    vix: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="VIX volatility index"
    )

    # Relationships
    analysis_run: Mapped["MacroAnalysisRun"] = relationship(
        "MacroAnalysisRun", back_populates="market_indicators"
    )
...
class EconomicIndicators(BaseModel):
    ...
    country: Mapped[str] = mapped_column(
        String(50), nullable=False, index=True, comment="Country code"
    )
    ...
    gdp_growth_qq: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="GDP growth QQ (%)"
    )

    gdp_growth_yy: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="GDP growth YY (%)"
    )

    unemployment: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Unemployment rate (%)"
    )

    inflation: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Inflation rate (%)"
    )

    industrial_production: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Industrial production growth (%)"
    )

    # Forecasts
    gdp_forecast_6m: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="GDP forecast 6M (%)"
    )

    inflation_forecast_6m: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Inflation forecast 6M (%)"
    )

    earnings_forecast_12m: Mapped[Optional[float]] = mapped_column(
        Float, nullable=True, comment="Earnings forecast 12M (%)"
    )

    # Relationships
    assessments: Mapped[List["CountryRegimeAssessment"]] = relationship(
        "CountryRegimeAssessment", back_populates="economic_indicators"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "unemployment IS NULL OR unemployment >= 0", name="ck_unemployment_positive"
        ),
        {
...
class CountryRegimeAssessment(BaseModel):
    ...
    rationale: Mapped[str] = mapped_column(
        Text, nullable=False, comment="Rationale for classification (2-3 sentences)"
    )
    ...
    recession_risk_6m: Mapped[float] = mapped_column(
        Float, nullable=False, comment="6-month recession risk (0.0-1.0)"
    )

    recession_risk_12m: Mapped[float] = mapped_column(
        Float, nullable=False, comment="12-month recession risk (0.0-1.0)"
    )

    recession_drivers: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="2-4 key recession risk factors"
    )

    # Portfolio positioning
    sector_tilts: Mapped[Dict[str, float]] = mapped_column(
        JSONB, nullable=False, comment="Sector weight adjustments (-0.10 to +0.10)"
    )

    recommended_overweights: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="3-5 sectors to overweight with rationale"
    )

    recommended_underweights: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="3-5 sectors to underweight with rationale"
    )
    ...
    primary_risks: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="2-3 main risks to monitor"
    )

    conflicting_signals: Mapped[List[str]] = mapped_column(
        JSONB, nullable=False, comment="Major contradictions (empty if none)"
    )

    # Relationships
    analysis_run: Mapped["MacroAnalysisRun"] = relationship(
        "MacroAnalysisRun", back_populates="country_assessments"
    )

    economic_indicators: Mapped["EconomicIndicators"] = relationship(
        "EconomicIndicators", back_populates="assessments", lazy="joined"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_assessment_sector_tilts_gin", "sector_tilts", postgresql_using="gin"
        ),
        Index("idx_assessment_risks_gin", "primary_risks", postgresql_using="gin"),
        UniqueConstraint(
            "country", "assessment_timestamp", name="uq_country_assessment_ts"
        ),
        CheckConstraint(
            "confidence >= 0.0 AND confidence <= 1.0", name="ck_confidence_range"
        ),
        CheckConstraint(
            "recession_risk_6m >= 0.0 AND recession_risk_6m <= 1.0",
            name="ck_recession_6m_range",
        ),
        CheckConstraint(
            "recession_risk_12m >= 0.0 AND recession_risk_12m <= 1.0",
            name="ck_recession_12m_range",
        ),
        {
...
class RegimeTransition(BaseModel):
    ...
    confidence: Mapped[float] = mapped_column(
        Float, nullable=False, comment="Transition confidence (0.0-1.0)"
    )

    days_since_last_transition: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, comment="Days since previous transition"
    )

    # Relationships
    assessment: Mapped["CountryRegimeAssessment"] = relationship(
        "CountryRegimeAssessment", back_populates="regime_transitions"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "confidence >= 0.0 AND confidence <= 1.0", name="ck_transition_confidence"
        ),
        CheckConstraint(
            "days_since_last_transition IS NULL OR days_since_last_transition >= 0",
            name="ck_days_positive",
        ),
        {
