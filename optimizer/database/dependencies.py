...
from optimizer.database.exceptions import AuthenticationError, RateLimitError

logger = logging.getLogger(__name__)

# Database session dependencies - imported from database module
from optimizer.database.database import get_db
    ...
    def _check_redis_rate_limit(
        ...
        try:
            ...
            if current_count >= self.requests:
                logger.warning(
                    f"⚠️  Rate limit exceeded for key {key}: {current_count}/{self.requests}"
                )
                from database.exceptions import RateLimitError
