...
async def calculate_robust_cross_sectional_stats(raw_data: List[dict]) -> dict:
    """Calculate robust cross-sectional statistics with iterative outlier removal."""
    market_caps = []
    ...
    cap_weights = (
        market_caps_arr / total_market_cap
        if total_market_cap > 0
        else np.ones(len(market_caps)) / len(market_caps)
    )
    ...
    def calculate_robust_stats(
        values: List[float], weights: np.ndarray, metric_name: str
    ...
    cross_sectional_stats = {"value": {}, "momentum": {}, "quality": {}, "growth": {}}
...
async def recalculate_with_cross_sectional_stats(
    ...
    """Recalculate z-scores using robust cross-sectional statistics."""
    from tqdm.asyncio import tqdm as async_tqdm

    raw_data_cs = []

    with async_tqdm(
        total=len(raw_data), desc="PASS 1B: Calculating z-scores", unit="stock"
    ) as pbar:
...
async def apply_cross_sectional_standardization(raw_data: List[dict], calculator) -> np.ndarray:
    """Apply winsorization + StandardScaler to raw z-scores."""
    ...
    z_standardized = calculator.standardize_zscores_cross_sectional(
        raw_zscores=raw_zscores, winsorize_threshold=10.0
    )

    return z_standardized


async def calculate_robust_factor_stats(raw_data: List[dict]) -> dict:
    """Calculate robust statistics for factor z-scores."""
...
async def analyze_factor_correlations(raw_data: List[dict]) -> Optional[np.ndarray]:
    """Analyze factor correlations and return correlation matrix."""
    # Extract all factor z-scores
    all_value_z = []
    all_momentum_z = []
    all_quality_z = []
...
async def classify_and_save_signals(
    ...
    """Classify signals using cross-sectionally standardized z-scores and save with momentum filters."""
    from tqdm.asyncio import tqdm as async_tqdm
    ...
    from src.stock_analyzer.adjustments.risk import (
    ...
    with async_tqdm(total=len(raw_data), desc="PASS 3: Saving signals", unit="stock") as pbar:
        with database_manager.get_session() as session:
            for i, item in enumerate(raw_data):
                ...
                try:
                    ...
                    stock_data, _, _ = await fetch_price_data(
                        instrument.yfinance_ticker, period="1y"
                    )
                    ...
                    signal_output = StockSignalOutput(
                        ...
                        open_price=(
                            float(stock_data["Open"].iloc[-1]) if "Open" in stock_data else None
                        ),
                        daily_return=(
                        ...
                        volume=(
                            float(stock_data["Volume"].iloc[-1]) if "Volume" in stock_data else None
                        ),
                    ...
                    return_1y = (
                        technical_metrics.get("annualized_return", 0.0)
                    ...
                    if return_1y < -0.15 and original_signal.value in [
                        "SMALL_GAIN",
                        "LARGE_GAIN",
                    ]:
