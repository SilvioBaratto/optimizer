...
class MacroRegimeDatabaseSaver:
    ...
    def save_analysis_run(
        self, all_results: Dict[str, Dict], fred_data: Dict, notes: Optional[str] = None
    ) -> uuid.UUID:
        """
    ...
    def save_single_country(
        self, country: str, results: Dict, run_id: Optional[uuid.UUID] = None
    ) -> None:
        """
    ...
    def _save_country_assessment(
        ...
        assessment_obj = CountryRegimeAssessment(
            ...
            yield_curve_signal=_normalize_signal_value(
                llm_classification.yield_curve_signal, "yield_curve"
            ),
            credit_spread_signal=_normalize_signal_value(
                llm_classification.credit_spread_signal, "credit_spread"
            ),
            # Recession risk
            recession_risk_6m=recession_risk_6m,
            recession_risk_12m=recession_risk_12m,
            recession_drivers=(
                llm_classification.recession_drivers if llm_classification.recession_drivers else []
            ),
            # Portfolio positioning
            sector_tilts=(
                llm_classification.sector_tilts if llm_classification.sector_tilts else {}
            ),
            recommended_overweights=(
                llm_classification.recommended_overweights
                if llm_classification.recommended_overweights
                else []
            ),
            recommended_underweights=(
                llm_classification.recommended_underweights
                if llm_classification.recommended_underweights
                else []
            ),
            factor_exposure=_normalize_factor_exposure(llm_classification.factor_exposure),
            # Risk monitoring
            primary_risks=(
                llm_classification.primary_risks if llm_classification.primary_risks else []
            ),
            conflicting_signals=(
                llm_classification.conflicting_signals
                if llm_classification.conflicting_signals
                else []
            ),
        )
    ...
    def _save_economic_indicators(
        self, session, country: str, ilsole_data: Dict, data_timestamp: datetime
    ) -> EconomicIndicators:
    ...
    def _save_regime_transition(
        self, session, assessment_id: uuid.UUID, country: str, transition_info: Dict
    ):
        ...
        with self.db_manager.get_session() as session:
            ...
            for country, data in trading_economics_data.items():
                ...
                if industrial_production:
                    ...
                    snapshot.industrial_production_reference = industrial_production.get(
                        "reference"
                    )
...
def save_analysis_to_database(
    all_results: Dict[str, Dict], fred_data: Dict, notes: Optional[str] = None
) -> uuid.UUID:
    """
