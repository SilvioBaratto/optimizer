...
class BusinessCycleClassifier:
    ...
    def _classify_regime(
        self, real, forecast, gdp_mom, inf_mom, ism, curve, hy
    ) -> Tuple[str, float, str, Dict]:
        # Base signals dictionary that ALWAYS includes momentum (for database storage)
        base_signals = {"gdp_momentum": gdp_mom, "inflation_momentum": inf_mom}
    ...
    def _calculate_recession_risk(self, real, forecast, gdp_mom, ism, curve, hy) -> Dict:
        ...
        if country_in_trouble:
            ...
            country_risk = (
                0.65 * trouble_severity
            )  # Up to 65% risk from country data (increased from 50%)
        ...
        global_weight = (
            0.8 if not country_in_trouble else 0.15
        )  # Increased healthy weight, reduced troubled weight
        ...
        # Cap at 95%
        return {"6month": min(0.95, risk_6m), "12month": min(0.95, risk_12m)}
    ...
    def classify_pure_llm(
        ...
        if self._baml_client is None:
            try:
                ...
                raise ImportError(
                    f"BAML client not available: {e}. Make sure baml_client/ exists in project root."
                )
        ...
        if (
            country_trading_economics_data
            and country_trading_economics_data.get("status") == "success"
        ):
        ...
        economic_indicators = EconomicIndicators(
            ...
            or te_indicators.get("gdp_growth_rate", {}).get("value"),
            gdp_growth_yy=real.get("gdp_yy")
            or te_indicators.get("gdp_growth_yoy", {}).get("value"),
            gdp_forecast_6m=forecast.get("gdp_growth_6m"),
            # Labor Market
            unemployment=real.get("unemployment")
            or te_indicators.get("unemployment_rate", {}).get("value"),
            # Production & Capacity
            industrial_production=real.get("industrial_production")
            or (te_industrial_prod.get("value") if te_industrial_prod else None),
            capacity_utilization=te_capacity_util.get("value") if te_capacity_util else None,
            # Inflation & Prices
            inflation=real.get("inflation")
            or real.get("consumer_prices")
            or te_indicators.get("inflation_rate", {}).get("value"),
        ...
        market_indicators = MarketIndicators(
            ...
            manufacturing_pmi=te_indicators.get("manufacturing_pmi", {}).get(
                "value"
            ),  # Country-specific
            ...
            yield_curve_2s10s=fred_data.get(
                "yield_curve_2s10s"
            ),  # From Trading Economics via run_regime_analysis
        ...
        try:
            # STEP 1: Summarize news articles
            news_signals = b.SummarizeNewsArticles(news=news_articles, country=country)
...
if __name__ == "__main__":
    # Test mode - requires live data from scrapers
    pass
