...
def fetch_historical_prices(
    positions: List[PortfolioPosition], start_date: datetime, end_date: datetime
) -> pd.DataFrame:
    """
    ...
    """
    logger.info(
        f"Fetching historical prices from {start_date.date()} to {end_date.date()}"
    )
    ...
    for position in positions:
        ...
        try:
            ...
            data = yf_client.fetch_history(
                yf_ticker, period=f"{days}d", min_rows=int(days * 0.5)
            )
...
def calculate_portfolio_returns(
    prices: pd.DataFrame, positions: List[PortfolioPosition]
) -> pd.Series:
    """
...
def fetch_benchmark_returns(
    start_date: datetime, end_date: datetime, benchmark_ticker: str = "^GSPC"
) -> pd.Series:
    """
    ...
    data = yf_client.fetch_history(
        benchmark_ticker, period=f"{days}d", min_rows=int(days * 0.5)
    )
...
def plot_performance(
    ...
    fig.suptitle(
        f"Portfolio Backtest: {portfolio_name} vs S&P 500",
        fontsize=16,
        fontweight="bold",
    )
    ...
    portfolio_drawdown = (
        portfolio_cumulative_full - portfolio_cumulative_full.expanding().max()
    ) / portfolio_cumulative_full.expanding().max()
    benchmark_drawdown = (
        benchmark_cumulative_full - benchmark_cumulative_full.expanding().max()
    ) / benchmark_cumulative_full.expanding().max()
...
def print_metrics_comparison(portfolio_metrics: Dict, benchmark_metrics: Dict):
    ...
    # Conclusion
    alpha = (
        portfolio_metrics["annualized_return"] - benchmark_metrics["annualized_return"]
    )
