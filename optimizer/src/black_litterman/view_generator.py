...
class ViewGenerator:
    ...
    def fetch_signals_from_db(
        ...
        with database_manager.get_session() as session:
            ...
            if signal_date:
                ...
                latest_date = (
                    session.query(StockSignal.signal_date)
    ...
    def fetch_macro_regime(self, country: str = "USA") -> Optional[MacroRegimeContext]:
        ...
        with database_manager.get_session() as session:
            ...
            regime = (
                session.query(CountryRegimeAssessment)
                .filter(CountryRegimeAssessment.country == country)
            ...
            indicators = (
                session.query(MarketIndicators)
                .order_by(MarketIndicators.data_timestamp.desc())
                .first()
            )
            ...
            macro_context = MacroRegimeContext(
                ...
                recession_risk_6m=(
                    float(regime.recession_risk_6m) if regime.recession_risk_6m else None
                ),
                recession_risk_12m=(
                ...
                hy_spread=(
                    float(indicators.hy_spread) if indicators and indicators.hy_spread else None
                ),
                yield_curve_2s10s=(
                    float(indicators.yield_curve_2s10s)
                    if indicators and indicators.yield_curve_2s10s
                    else None
                ),
    ...
    def build_sector_context(self, sector: str, signals: List[StockSignal]) -> SectorContext:
        """
    ...
    async def _generate_single_view(
        ...
        try:
            ...
            yfinance_ticker = signal.yfinance_ticker or (
                instrument.yfinance_ticker if instrument else ticker
            )
            ...
            signal_data = StockSignalData(
                ...
                confidence_level=(
                    ...
                    if signal.confidence_level and hasattr(signal.confidence_level, "value")
                    else "MEDIUM"
                ),
                ...
                volatility_level=(
                    signal.volatility_level.value if signal.volatility_level else None
                ),
                ...
                upside_potential_pct=(
                    float(signal.upside_potential_pct) if signal.upside_potential_pct else None
                ),
                downside_risk_pct=(
                    float(signal.downside_risk_pct) if signal.downside_risk_pct else None
                ),
                ...
                if news_data and len(news_data) > 0:
                    ...
                    for article in news_data:
                        ...
                        news_article = NewsArticle(
                            ...
                            full_content=article.get(
                                "full_content"
                            ),  # Full article text from web scraping
                        )
                        news_articles.append(news_article)

                    logger.debug(
                        f"Converted {len(news_articles)} articles to BAML format for {ticker}"
                    )

                    # STEP 2: Summarize news using BAML
                    logger.debug(f"Summarizing {len(news_articles)} articles for {ticker}...")
                    news_signals = b.SummarizeStockNews(ticker=ticker, news=news_articles)
                    logger.debug(
                        f"News summary complete for {ticker}: {news_signals.news_bias} bias"
                    )

                else:
                    logger.debug(
                        f"No news articles found for {ticker}, proceeding without news context"
                    )
    ...
    async def generate_views(
        self, signal_date: Optional[date_type] = None, country: str = "USA"
        ...
        if not macro_regime:
            ...
            macro_regime = MacroRegimeContext(current_regime="UNCERTAIN", regime_confidence=0.5)

        # Generate views using BAML
        views = []
        for signal in signals:
            # Build sector context
            sector_context = self.build_sector_context(signal.sector or "Unknown", signals)
    ...
    def construct_matrices(
        ...
        for k, (signal, view) in enumerate(views):
            # Get ticker index (use Trading212 ticker to match universe_tickers)
        ...
        logger.info(f"Constructed BL matrices: P({K}x{N}), Q({K}x1), Omega({K}x{K})")
    ...
    def construct_basket_views(
        ...
        for k, (group_name, group_views) in enumerate(groups.items()):
            # Get tickers in this group (use Trading212 ticker to match universe_tickers)
            group_tickers = [
                signal.ticker for signal, _ in group_views if signal.ticker in ticker_index
            ]
    ...
    def summary_stats(self, views: List[Tuple[StockSignal, BlackLittermanView]]) -> Dict:
        ...
        stats = {
            ...
            "sectors": {
                sector: {"count": len(rets), "avg_return": np.mean(rets)}
