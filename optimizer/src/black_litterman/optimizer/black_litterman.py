...
class BlackLittermanModel(base_optimizer.BaseOptimizer):
    ...
    def _parse_views(
        self, absolute_views: Union[Dict[str, float], pd.Series]
    ...
    def _set_pi(
        self, pi: Optional[Union[str, np.ndarray, pd.Series, pd.DataFrame]], **kwargs
    ) -> None:
        if pi is None:
    ...
    @staticmethod
    def default_omega(cov_matrix: np.ndarray, P: np.ndarray, tau: float) -> np.ndarray:
        """
    ...
    def bl_weights(self, risk_aversion: Optional[float] = None) -> "OrderedDict":
        r"""
    ...
    def optimize(self, risk_aversion: Optional[float] = None) -> "OrderedDict":
        """
    ...
    def portfolio_performance(
        ...
        if self.posterior_rets is None:
            raise ValueError(
                "Posterior returns not computed. This should not happen after bl_weights()."
            )
