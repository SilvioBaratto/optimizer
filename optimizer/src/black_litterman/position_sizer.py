...
def fetch_current_prices(positions: List[PortfolioPosition]) -> Dict[str, float]:
    ...
    if failed_tickers:
        logger.warning(
            f"Failed to fetch prices for {len(failed_tickers)} tickers: {failed_tickers}"
        )
...
def calculate_position_sizes(
    ...
    for position in positions:
        ...
        if actual_amount < min_position_value:
            logger.debug(
                f"Skipping {ticker}: amount €{actual_amount:.2f} below minimum €{min_position_value:.2f}"
            )
            continue
    ...
    logger.info(
        f"✓ Allocated {len(df)} positions using €{total_invested:.2f} of €{total_capital:.2f} ({(total_invested/total_capital)*100:.1f}%)"
    )
...
def print_trading_plan(df: pd.DataFrame, total_capital: float):
    ...
    print("\n" + "-" * 140)
    print(
        f"{'#':<3} {'Ticker':<15} {'Company':<30} {'Price':>12} {'Shares':>14} {'Cost':>14} {'Weight%':>10}"
    )
    print("-" * 140)
    ...
    print(
        f"{'Remaining Cash:':<25} €{remaining_cash:>12,.2f} ({remaining_cash/total_capital:>6.2%} of capital)"
    )
...
def main():
    ...
    sizing_df = calculate_position_sizes(
        positions, prices, args.capital, min_position_value=args.min_position
    )
